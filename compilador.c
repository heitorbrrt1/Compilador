/**
 * FASE 1 - Analisador Léxico e Controle de Memória
 *
 * Este programa implementa a primeira fase de um compilador para uma linguagem de programação customizada,
 * conforme as especificações do documento "CompiladorEspecificacao.pdf".
 *
 * Funcionalidades implementadas:
 * 1.  Controle de Memória:
 * - Alocação, liberação e monitoramento do uso de memória.
 * - Emite alertas quando o uso de memória atinge 90% e erros fatais em 100%.
 * - A memória total é parametrizável.
 *
 * 2.  Análise Léxica:
 * - Reconhecimento de tokens da linguagem.
 * - Identificação de palavras reservadas (ex: 'inteiro', 'se', 'principal').
 * - Identificação de variáveis (iniciadas com '!').
 * - Identificação de nomes de funções (iniciados com '').
 * - Identificação de literais numéricos (inteiros e decimais) e de texto.
 * - Identificação de operadores (matemáticos, relacionais, lógicos) e delimitadores.
 *
 * Como compilar e usar:
 * 1.  Salve este código como 'analisador.c'.
 * 2.  Crie um arquivo de texto chamado 'codigo_fonte.txt' com o código a ser analisado.
 * 3.  Compile usando um compilador C (GCC, por exemplo):
 * gcc analisador.c -o analisador
 * 4.  Execute o programa:
 * ./analisador
 *
 * O programa lerá o 'codigo_fonte.txt', processará os tokens e imprimirá o resultado
 * da análise, junto com o status da memória.
 */
...
TipoToken verificar_palavra_reservada(const char* str) {
    ...
    return TOKEN_ERRO; // Não é uma palavra reservada
}
    ...
    while ((c = proximo_char()) != EOF) {
        ...
        if (isdigit(c)) {
            ...
            while ((isdigit(c = proximo_char()) || c == '.') && i < 255) {
                if (c == '.') { if (tem_ponto) break; tem_ponto = 1; }
                buffer[i++] = c;
            }
            devolver_char(c);
            buffer[i] = '\0';
        ...
        if (isalpha(c) || c == '_') {
            ...
            if (strncmp(buffer, "", 2) == 0) {
                 if (strlen(buffer) > 2 && isalpha(buffer[2])) return criar_token(TOKEN_ID_FUNCAO, buffer, linha_atual);
            }
